...
function getPublicIp(){
    fetch("https://geolocation-db.com/json/", {
        ...
        .then((data) => {
            console.log(data);
            currentCity = "Kirklees";
            getWeatherData("Kirklees", currentUnit, hourlyorWeek);
        });
}
getPublicIp();
...
function getWeatherData(city,unit, hourlyorWeek) {
    const apiKey = "J3YZ9BXYBEXRR9ERZ7HBEEPFT";
    ...
    .then((data) => {
        ...
        sunRise.innerText = convertTimeTo12HhourFormat(today.sunrise);
        sunSet.innerText = convertTimeTo12HhourFormat(today.sunset);
...
//function to update the  humidity status
...
//function to update the air quality status
...
function convertTimeTo12HhourFormat(time) {
    let hour = time.split(":")[0];
    let minute = time.split(":")[1];
    let ampm = hour >= 12 ? "pm" : "am";
    hour = hour & 12;
    hour = hour ? hour : 12; // the zero hour should be 12 not working properly
...
function getIcon(condition) {
    ...
    else if (condition === "parlty-cloudy-night") {
...
//cities array for suggestions (any API)
cities = [
    "Pretoria",
    "Johannesburg",
    "Cape Town",
...
search.addEventListener("input", function(e) {
    ...
    //append the ul to its parent whidh is search form
    ...
    for (i =0; i < cities.length; i++) {
        ...
        if (cities[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            //strong to make matching letters bold
...
//function to remove previous suggestion and replace with new one
function removeSuggestions() {
    //select the ul which is being added as search input
...
search.addEventListener("keydown", function (e) {
    ...
    if (e.keyCode == 40) {
        ...
        currentFocus++;
        //function to add active suggestion
        addActive(x);
    }
    ...
    if (e.keyCode == 13) {
        //if enter is pressed add the current select suggsetion in input field
...
function addActive(x) {
    ...
    //if current focus is more than the length of suggestion array ake it 0
    if (currentFocus >= x.length) currentFocus = 0;

    //if its less than 0 make it last suggestion equal
